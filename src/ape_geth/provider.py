...
class BaseGethProvider(Web3Provider):
    ...
    def _log_connection_complete(self):
...
class GethDev(TestProviderAPI, BaseGethProvider):
    ...
    def revert(self, snapshot_id: SnapshotID):
        if isinstance(snapshot_id, int):
            ...
            block_number = HexStr(str(snapshot_id))
    ...
    def snapshot(self) -> SnapshotID:
        ...
        if latest_block_number is None:
            # Should never happen, but for mypy's sake.
            raise ProviderError("Latest block number is not available.")
